...
from sales_pipeline.db.models import Lead
from sales_pipeline.db.repository import (
    create_reply,
    get_last_email_for_lead,
    get_leads,
    update_lead_status,
)
from sales_pipeline.email_sender.gmail_client import _get_gmail_service
...
def check_lead_replies(db: Session, service: Any, lead: Lead) -> bool:
    ...
    last_sent = get_last_email_for_lead(db, lead.id)
    if not last_sent:
    ...
    try:
        ...
        for msg in messages:
            ...
            if received_at > sent_at_utc:
                ...
                update_lead_status(db, lead.id, "replied")
...
def check_new_replies() -> None:
    ...
    with get_db() as db:
        ...
        contacted_leads = get_leads(db, status="contacted", limit=100)
        ...
        if replies_found:
            try:
                from sales_pipeline.crm.sheets_client import sync_data_to_sheets
