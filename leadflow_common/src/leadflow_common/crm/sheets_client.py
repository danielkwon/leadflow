...
from leadflow_common.db.models import Lead, Reply
from leadflow_common.db.repository import get_leads, get_unsynced_replies, mark_reply_synced
...
def sync_data_to_sheets() -> None:
    ...
    with get_db() as db:
        # === 1. 리드 현황 동기화 ===
        leads = get_leads(db, user_id=1, limit=2000)  # 최대 2000개 리드 동기화 지원
        lead_rows = []
        for l in leads:
            lead_rows.append([
                l.id,
        ...
        for r in all_replies:
            ...
            if not r.synced_to_sheet:
                mark_reply_synced(db, r.id, user_id=1)
