...
from leadflow_common.email_sender.gmail_client import send_email
...
def run_campaign_by_name(name: str) -> dict[str, int]:
    ...
    with get_db() as db:
        campaign = get_campaign_by_name(db, user_id=1, name=name)
        if not campaign:
        ...
        # 발송 대상자(리드) 조회
        candidates = get_drip_candidates(db, user_id=1, campaign=campaign)
        ...
        for lead in candidates:
            ...
            try:
                ...
                update_lead_status(db, lead.id, user_id=1, status="contacted")
