...
from leadflow_common.db.repository import get_leads, update_lead_email
...
def enrich_leads_with_email(
    ...
    with get_db() as db:
        ...
        leads = get_leads(db, user_id=1, has_email=False, limit=limit)
        ...
        for lead in leads_with_url:
            ...
            if email:
                update_lead_email(db, lead.id, user_id=1, email=email)
                result["found"] += 1
            else:
                result["failed"] += 1
