...
from leadflow_common.db.repository import create_lead, find_duplicate_lead
from leadflow_common.utils.logging import get_logger

log = get_logger("scraper.naver_map")

# 전역 수집 상태 관리 매니저 (실시간 SSE/AJAX 피드백 연동용)
SCRAPE_PROGRESS = {}


def get_scrape_progress(user_id: int) -> dict:
    """해당 사용자의 현재 수집 진행 상태를 실시간 반환한다."""
    return SCRAPE_PROGRESS.get(user_id, {
        "status": "idle",
        "added": 0,
        "max_results": 0,
        "message": "수집 대기 중"
...
def fetch_leads_via_playwright(
    ...
    log.info("Playwright 리스너 기반 크롤러 기동...", query=query, user_id=user_id)
    SCRAPE_PROGRESS[user_id] = {
        "status": "running",
        "added": 0,
        "max_results": max_results,
        "message": "Playwright 가상 브라우저 인스턴스 초기화 중..."
    }
    ...
    with sync_playwright() as p:
            ...
            if "api/search/allSearch" in url and response.status == 200:
                try:
                    ...
                    if result.get("ncaptcha"):
                        ...
                        SCRAPE_PROGRESS[user_id] = {
                            "status": "failed",
                            "added": len(collected_items),
                            "max_results": max_results,
                            "message": "네이버 지도 CAPTCHA 보안 필터 감지 (IP 우회 또는 대기 필요)"
                        }
                        return
                    ...
                    if added_count > 0:
                        ...
                        SCRAPE_PROGRESS[user_id] = {
                            ...
                            "added": len(collected_items),
                            "max_results": max_results,
                            "message": f"실시간 네이버 지도 API 응답 수집 중... ({len(collected_items)}개 업체 발견)"
                        }
        ...
        try:
            ...
            if iframe_element:
                ...
                SCRAPE_PROGRESS[user_id]["message"] = "검색 리스트 프레임(searchIframe) 마운트 감지. 순차적 페이징 수집 개시..."
                ...
                while len(collected_items) < max_results:
                    ...
                    if page_btn.is_visible():
                        ...
                        SCRAPE_PROGRESS[user_id]["message"] = f"UI 조작 시뮬레이션: 검색 {current_page}페이지 이동 링크 클릭 중..."
                        page_btn.click()
                        # 로딩 안정화 시간 부여
...
def scrape_leads(
    ...
    log.info("비공식 웹 브라우저 크롤링 파이프라인 시작", user_id=user_id, region=region, keyword=keyword, max_results=max_results)
    SCRAPE_PROGRESS[user_id] = {
        "status": "running",
        "added": 0,
        "max_results": max_results,
        "message": f"'{region} {keyword}' 실시간 스크래핑 세션 예약 개시..."
    }
    
    try:
        items = fetch_leads_via_playwright(user_id=user_id, query=query, max_results=max_results)
    except Exception as e:
        ...
        SCRAPE_PROGRESS[user_id] = {
            ...
            "added": 0,
            "max_results": max_results,
            "message": f"Playwright 크롤러 기동 실패: {str(e)}"
        }
        result["errors"] += 1
        return result
    ...
    if not items:
        ...
        SCRAPE_PROGRESS[user_id] = {
            ...
            "added": 0,
            "max_results": max_results,
            "message": f"'{region} {keyword}' 검색 결과가 네이버 지도에 존재하지 않거나 수집되지 않았습니다."
        }
        return result
    ...
    with get_db() as db:
        for item in items:
            try:
                ...
                # 중복 확인 (사용자별 범위 격리)
                existing = find_duplicate_lead(db, user_id, company_name, phone if phone else None)
                if existing:
                    result["skipped"] += 1
                    log.debug("중복 리드 스킵", company=company_name, user_id=user_id)
                    continue
                ...
                log.error("리드 저장 실패", error=str(e), company=item.get("name"), user_id=user_id)
                result["errors"] += 1
                
    log.info("스크래핑 완료", region=region, keyword=keyword, result=result, user_id=user_id)
    SCRAPE_PROGRESS[user_id] = {
        "status": "completed",
        "added": result["added"],
        "max_results": max_results,
        "message": f"'{region} {keyword}' 수집 및 동기화 완료! 총 {result['added']}건 추가됨 (중복 스킵: {result['skipped']}건)"
    }
    return result
