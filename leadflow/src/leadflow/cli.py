...
from leadflow.main import bootstrap
...
def leads_list(
    ...
    from leadflow.db.repository import count_leads, get_leads
...
def leads_stats() -> None:
    """리드 현황 통계를 표시한다."""
    from leadflow.db.engine import get_db
    from leadflow.db.repository import count_leads
...
def scrape_run(
    ...
    from leadflow.scraper.naver_api import scrape_leads
...
def scrape_enrich(
    ...
    from leadflow.scraper.website_scraper import enrich_leads_with_email
...
def email_send_test(
    ...
    from leadflow.email_sender.gmail_client import send_test_email
...
def campaign_run(
    ...
    from leadflow.campaign.drip import run_campaign_by_name
...
def campaign_create(
    ...
    """새 캠페인을 생성한다."""
    from leadflow.db.engine import get_db
    from leadflow.db.repository import create_campaign
...
def scheduler_start() -> None:
    ...
    from leadflow.campaign.scheduler import start_scheduler
...
def monitor_start() -> None:
    ...
    from leadflow.monitor.inbox_watcher import start_monitoring
...
@app.command("web")
def web_start(
    host: str = typer.Option("127.0.0.1", help="호스트 주소"),
    port: int = typer.Option(8000, help="포트 번호"),
) -> None:
    """LeadFlow SaaS 웹 대시보드를 시작합니다."""
    import uvicorn
    typer.echo(f"🚀 LeadFlow SaaS 대시보드 구동 중... http://{host}:{port}")
    # PYTHONPATH를 src로 주기 위해 python module import 방식
    uvicorn.run("leadflow.web.app:app", host=host, port=port, reload=True)
